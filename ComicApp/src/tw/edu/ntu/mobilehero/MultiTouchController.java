...
public class MultiTouchController<T> {
    ...
    public static interface OnFlickListener<T> {
        abstract void onFlick(T obj);
        abstract void onFingerUp(T obj, MotionEvent event);
        abstract void onDrag(T obj, MotionEvent event);
        abstract void onFingerDown(T obj, MotionEvent event);
    }
    ...
    private OnFlickListener<T> mOnFlick;
     ...
     */
    public MultiTouchController(MultiTouchObjectCanvas<T> objectCanvas, ViewConfiguration viewConfiguration) {
        this(objectCanvas, true, viewConfiguration);
    }
    ...
    public MultiTouchController(MultiTouchObjectCanvas<T> objectCanvas,
        ...
        minVelocity = viewConfiguration.getScaledMinimumFlingVelocity() * 16;
        maxVelocity = Math.max(viewConfiguration.getScaledMaximumFlingVelocity(), minVelocity) * 2;
    }
    ...
    private static final int FLICK_TIME = 1000;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        T oldSelectedObject = selectedObject;
        try {
            pointerCount = multiTouchSupported ? (Integer) m_getPointerCount
                    .invoke(event) : 1;
            ...
            /*
             * This is to detect when the velocity is high
             */
            //If selecting background, then cancel
            ...
            switch (action) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    mOnFlick.onFingerDown(selectedObject, event);
                    break;
                case MotionEvent.ACTION_MOVE:
                    vTracker.addMovement(event);
                    mOnFlick.onDrag(selectedObject,event);
                    break;
                case MotionEvent.ACTION_UP:
                    vTracker.addMovement(event);
                    vTracker.computeCurrentVelocity(FLICK_TIME, maxVelocity);
                    float x = vTracker.getXVelocity();
                    float y = vTracker.getYVelocity();
                    float velocity = (float)Math.sqrt(x * x + y * y);
                    if (velocity >= minVelocity && mOnFlick != null)
                        mOnFlick.onFlick(oldSelectedObject);
                    else
                        mOnFlick.onFingerUp(oldSelectedObject, event);
    ...
    private void performDragOrPinch() {
        ...
        // Set the new obj coords, scale, and angle as appropriate (notifying
        // the subclass of the change).
    ...
    public static class PositionAndScale {
         ...
         */
        public void set(float xOff, float yOff, float scale, float scaleX,
    ...
    public static interface MultiTouchObjectCanvas<T> {
         ...
         */
        public void setPositionAndScale(T obj,
    ...
    public void setOnFlickListener(OnFlickListener<T> onFlick) {
        mOnFlick = onFlick;
    }
